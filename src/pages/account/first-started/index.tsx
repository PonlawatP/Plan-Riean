...
function ProfileFSPage(props:any){
                ...
                if(d.enabled){
                    uni.push(
                        <span
                            ...
                            flex pl-6 gap-4 items-center text-sm duration-300 text-pr-blue-dark ${firstStepData.uni_id == d.uni_id ? "border-2 border-pr-msu-1" : ""}`}
                        >
                            {d.uni_logo ? 
                                <Image
                                src={d.uni_logo}
    ...
    function getCourseSetRelation(): Array<any>{
        ...
        const same_year = getStudentEducatedYear(firstStepData.std_id) == (Number.isNaN(firstStepData.std_year) ? 1 : firstStepData.std_year)
        const coursesets = uniFacData.find((f:any)=>f.fac_id==firstStepData.fac_id)?.coursesets[0].children || [];
        const std_year = firstStepData.std_id != "" ? Number.parseInt(firstStepData.std_id.substring(0,2)) + (!same_year ? getStudentEducatedYear(firstStepData.std_id) - ((Number.isNaN(firstStepData.std_year) ? 1 : firstStepData.std_year) as number || 0) : 0) : (new Date(Date.now()).getFullYear() + 543 - 2500) - getStudentEducatedYear(firstStepData.std_id)
        ...
        const res_lower = coursesets.filter((c:any)=>{
            ...
            content: <>
                ...
                <form className="text-sm flex flex-col gap-4 mt-4">
                    ...
                    <div className="mt-12 relative h-10">
                        ...
                        >
                            {uniFacData.map((f:any, find:number)=><option key={find} value={f.fac_id}>{f['fac_name_'+'th']}</option>)}
                        </select>
                        <label
                ...
                <div className="grid grid-cols-[7em_auto] gap-4">
                    <div className="mt-8 relative h-10">
                        <input
                            ...
                            onChange={(e)=>{
                                const stdy = Number.parseInt(e.target.value) <= 0 || Number.parseInt(e.target.value) >= 100 ? 1 :Number.parseInt(e.target.value)
                    ...
                    <div className="mt-8 relative h-10">
                        <select 
                            ...
                            {getCourseSetRelation().map((c:any, cind:number)=><option key={cind} value={c.cr_id}>{`${c.cr_key} - ${c['name_'+'th']}`}</option>)}
                        </select>
                        <label
                            className="before:content[' '] after:content[' '] pointer-events-none absolute left-0 -top-1.5 flex h-full w-full select-none text-[11px] font-normal leading-tight text-blue-gray-400 transition-all before:pointer-events-none before:mt-[6.5px] before:mr-1 before:box-border before:block before:h-1.5 before:w-2.5 before:rounded-tl-md before:border-t before:border-l before:border-blue-gray-200 before:transition-all after:pointer-events-none after:mt-[6.5px] after:ml-1 after:box-border after:block after:h-1.5 after:w-2.5 after:flex-grow after:rounded-tr-md after:border-t after:border-r after:border-blue-gray-200 after:transition-all peer-placeholder-shown:text-sm peer-placeholder-shown:leading-[3.75] peer-placeholder-shown:text-blue-gray-500 peer-placeholder-shown:before:border-transparent peer-placeholder-shown:after:border-transparent peer-focus:text-[11px] peer-focus:leading-tight peer-focus:text-gray-900 peer-focus:before:border-t-2 peer-focus:before:border-l-2 peer-focus:before:border-gray-900 peer-focus:after:border-t-2 peer-focus:after:border-r-2 peer-focus:after:border-gray-900 peer-disabled:text-transparent peer-disabled:before:border-transparent peer-disabled:after:border-transparent peer-disabled:peer-placeholder-shown:text-blue-gray-500">
            ...
            <div className="flex flex-col justify-between h-full">
                <div className="relative">
                    ...
                    <div className={`smooth-all h-10 w-10 absolute right-0 top-2 flex justify-center items-center ${step.index+1 == stepContent.length ? "opacity-0":"opacity-100"}`}>
                        ...
                        <CircularProgressbar 
                            value={step.index+1} strokeWidth={15} maxValue={stepContent.length}
                            styles={buildStyles({
