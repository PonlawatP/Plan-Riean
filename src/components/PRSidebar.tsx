...
export default function PRSidebar(props: any) {
  const { hidden } = props;
  const { viewSchedule, topbarToggle, getCurrentPlan, createNewPlan } = useContext(CalendarContext);
  const { handleOpenSubjectSelect } = useContext(CalendarFilterContext);
    ...
    <section
      className={`
        pr-sidebar smooth-all relative hidden p-8 drop-shadow-xl min-h-[460px] md:w-[122px] ${
          !viewSchedule ? 'lg:w-[260px]' : 'md:w-[510px] lg:w-[510px]'
        ...
        <div className="content relative flex flex-col h-full bg-white/90 border-[1px] rounded-3xl overflow-hidden">
          {redirect.pathname == '/plan/plan/[plan_id]' ? (
            <>
              ...
              <button
                className={`px-4 h-14 text-left flex items-center gap-2 group ${getBarActive('/plan/plan/[plan_id]')}`}
              >
