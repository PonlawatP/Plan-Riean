  ...
  return (
    <section
      className={`pr-subject-filter pointer-events-none smooth-all absolute lg:left-[465px] md:p-8 w-full md:w-auto bottom-0 h-full grid z-10 lg:z-auto ${
        !viewSchedule || topbarToggle.init || topbarToggle.pre || !viewFilter || !isOn
      ...
      {isOn || tempOn ? (
        ...
        >
          ...
          <section className="pr-subject-header flex justify-between p-2 py-3 border-b-[1px] border-slate-400/50">
            ...
            {search ? (
              <div className="relative text-white/80 focus-within:text-gray-400 mr-2">
                ...
                <input
                  onChange={(e) =>
                    f.handleFilterOnSubmit(() => {
                      onSearch(e.target.value);
                    })
                  }
                  type="search"
                  name="q"
                  className="w-36 py-2 text-sm bg-pr-bg-3 focus:bg-pr-bg focus:text-pr-msu-2 border-slate-600/50 border-b-[3px] rounded-md pl-10 pr-2 focus:outline-none"
          ...
          <div className="content w-full h-full relative grid grid-rows-[1fr_auto]">
            <span
              className={`smooth-all absolute pointer-events-none block h-1 shadow-md left-0 rounded-b-full shadow-pr-msu-1 bg-pr-msu-1`}
              style={{
                opacity: `${f.filterDebounceProgress > 96 || f.filterDebounceProgress == 0 ? 0 : 100}%`,
                width: `${f.filterDebounceProgress}%`,
              }}
            ></span>
            <section className="pr-subject-filter-content relative overflow-auto fade-y">{props.children}</section>
