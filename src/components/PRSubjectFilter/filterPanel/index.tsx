          ...
          <div className="content w-full h-full relative grid grid-rows-[1fr_auto]">
            ...
            <section className="pr-subject-actions z-10 p-5 pt-2">
              <div className="pr-filter-btn flex gap-3 justify-end">
                <button onClick={()=>f.resetSubjectViewFilter()} className={`w-24 text-pr-text-menu h-fit px-2 py-1 rounded-lg bg-pr-bg border-b-[3px] border-slate-400/50 hover:bg-slate-300 active:border-0 active:bg-slate-400 active:text-white/80 ${calsel_data.isLoading ? "pointer-events-none opacity-50" : ""}`}>
                  ล้าง
                </button>
                <button onClick={()=>f.handleSubjectFilterSubmit()} className={`w-24 h-fit px-2 py-1 rounded-lg border-b-[3px] active:border-0 text-white/80 bg-pr-bg-3 border-slate-600/50 hover:bg-slate-600 active:bg-slate-600 active:text-white/80 ${calsel_data.isLoading ? "pointer-events-none opacity-50" : ""}`}>
                  ตกลง
                </button>
              </div>
            </section>
