...
export default function Home({ props }: any) {
  ...
  useEffect(() => {
    if (
      ...
      localStorage.getItem("plan_index") != null
    ) {
            ...
            getSplitedData(subject.time).filter(
              (sf: IsubjectSectDate) =>
                ...
                (getHourIndex(df.fullDate) == getHourIndex(sf.fullDate) ||
                  (getHourIndex(sf.fullDate) +
                    (calculateScale(sf.fullDate) - 1) >
                    getHourIndex(df.fullDate) &&
                    getHourIndex(df.fullDate) +
                      (calculateScale(df.fullDate) - 1) >
                      getHourIndex(sf.fullDate)))
            ).length > 0
        ).length > 0
    );
  ...
  function SubjectCard(props: any) {
    ...
    return (
      <div
        className={`mt-3 min-h-[5rem] rounded-xl overflow-hidden bg-slate-100 relative border-[2px] cursor-pointer border-black/10`}
        onClick={fnHandleClickedCard}
      >
        <span className="flex absolute left-0 top-0">
          <span
            className={`px-3 py-[2px] border-b-2 border-r-2 border-black/20 rounded-br-xl text-black/90 ${getColorCode(
              data.code
            )}`}
          >
            <h3 className="text-center opacity-80 text-[13px]">
              หมวดหมู่ที่ {data.type.split("-")[1]}
            </h3>
          </span>
          ...
          <div className="">
            <span className="flex gap-4 items-center pt-1 relative">
              <span className="bg-slate-400/30 px-2 rounded-lg">
                ทั้งหมด {data.detail.opened}
              </span>
              <span className="bg-slate-400/30 px-2 rounded-lg">
                หมวดย่อย {data.detail.sub}
              </span>
              <span className="bg-slate-400/30 px-2 rounded-lg">
                บังคับ {data.detail.forced}
              </span>
  ...
  function SubjectSectCard(props: any) {
    ...
    return (
      <div
        className={`mt-3 min-h-[5rem] rounded-xl overflow-hidden flex items-end bg-slate-100 relative border-[2px] cursor-pointer ${
          checkSubjectSchedule(data)
            ? "border-green-400/90 shadow-green-400/40 shadow-md"
            : "border-black/10"
        } ${
          checkSubjectCollapsed(data) && !checkSubjectSchedule(data)
            ? "opacity-40 brightness-75"
            : "opacity-100 brightness-100"
        }`}
        onClick={() => {
          fnHandleClickedSubjectCard(data);
        }}
      >
        ...
        <div className="pt-[1.8rem] pb-1 px-2 w-full text-sm">
          ...
          {data.lecturer.split(" / ").map((lect: any, lindex: any) => (
            <p key={lindex} className="text-black/40">
              {lect}
            </p>
          ))}
          <div className="">
            {dateData.map((date, dateindex) => (
              <span
                key={dateindex}
                className="flex gap-4 items-center pt-1 relative"
              >
                <span
                  className={
                    date.dayColor + " px-2 rounded-lg text-center w-16"
                  }
                >
                  {date.dayName}
                </span>
  ...
  const SubjectSectCardList = subjectShowData.map((data: any, index: any) => (
    <div
      key={index}
      className={`${index === subjectShowData.length - 1 ? "mb-3" : ""}`}
    >
  ...
  async function updateSubjectGroupList() {
    ...
    try {
      await ge_subject_group_name.forEach((ge, index) => {
        getGroupOfSubjectData(ge.type, abortSubjController.signal).then(
          (res) => {
  ...
  const ScheduleCard = (props: any) => {
    ...
    return (
      <div className="absolute w-full h-full">
        <div
          className={`relative h-full z-40 p-2 group`}
        ...
        >
          <div
            ...
            className={`z-10 relative cursor-pointer rounded-lg border-2 border-white/25 h-full w-full p-1 text-center shadow-md hover:text-white/95 ${getColorCode(
              data.code
  ...
  return (
    <>
      ...
      <div
        className={`transition-all duration-1000 w-full h-full relative ${
          state.viewSchedule ? "bg-black/70" : "bg-slate-300"
        }`}
      >
        {/* introduce */}
        <div
          className={`smooth absolute w-full top-3 left-3 z-50 ${
            state.viewSchedule ? "opacity-0 pointer-events-none" : ""
          }`}
        >
          ...
          <a
            ...
            href="https://forms.gle/VuMvvSLD2FtZm3Mn7"
          >
        ...
        {/* slide up overlay */}
        <div
          className={`absolute bottom-5 w-full flex flex-col gap-3 justify-center items-center rounded-full z-50 pointer-events-none lg:hidden ${
            state.viewSchedule && "hidden"
          }`}
        >
          <p
            className={`smooth text-slate-500 ${
              state.webReady ? "opacity-100" : "opacity-0"
            } ${filter.firstFilter && "hidden"}`}
          >
            เลื่อนขึ้นเพื่อดูรายวิชา
          </p>
          <FontAwesomeIcon
        ...
        {/* calendar button */}
        <div
          className={`hidden lg:flex justify-center absolute bottom-5 w-full z-50 ${
            state.viewSchedule && "hidden"
          }`}
          onClick={() => toggleScheduleSpectate(true)}
        >
          <div className="h-12 px-4 flex gap-2 justify-center items-center rounded-2xl shadow-lg bg-slate-200 hover:bg-slate-300 cursor-pointer">
            ...
            <span className="text-sm text-black/60">เลือกวิชาเรียน</span>
          </div>
        </div>

        {/* Summary Calendar section */}
        <div
          className={`smooth-out ${
            state.viewSchedule
              ? "h-[35dvh] overflow-hidden"
              : "w-full h-[100dvh]"
            ...
            >
              ...
              <div className="pl-16 inline-flex ">
                {times_m.map((time, tindex) => (
                  <span
                    key={tindex}
                    className="h-full flex items-center w-24 py-2 pl-3 border-l-2 border-b-2 border-black/5 bg-fuchsia-300 text-orange-950/40"
                  >
              ...
              {name_days.map((d, dindex) => (
                <div key={dindex} className="pl-16 inline-flex">
                  {times_m.map((time, tindex) => (
                    <span
                      key={dindex + "-" + tindex}
                    ...
                    >
                      ...
                      {state.webReady &&
                        ((tindex % 2 == 0 && tindex < 4) ||
                          ...
                          >
                            <div
                              className={`relative h-full p-1 group z-10`}
                              style={{ width: "200%" }}
                            >
                              <div
                                onClick={() =>
                                  fnHandleClickedOnCalendar(tindex, dindex)
                                }
                                className={`cursor-pointer rounded-lg border-2 border-white/25 h-full w-full p-1 text-center shadow-md text-white/95 bg-black/40 opacity-0 transition-all duration-500 flex items-center justify-center ${
                                  !state.viewSchedule && "hover:opacity-100"
                                } hover:duration-100`}
                              >
                      ...
                      {getCurrentPlan().data.map(
                        (data: any, dataindex: any) => {
                          return getSplitedData(data.time).map(
                            (split_date, spindex) => {
                              return dindex ==
                                getDayIndex(split_date.fullDate) &&
        ...
        <div
          className={`fixed smooth-out overflow-hidden w-full h-[65dvh] ${
            state.viewSchedule ? "bottom-0" : "-bottom-full"
          } bg-white rounded-t-3xl z-50`}
          style={{
            bottom:
              (!state.viewSchedule
                ? -65 - state.swipedLocated
                : 0 - state.swipedLocated < 0
                ? 0 - state.swipedLocated
                : 0) + "%",
          }}
        >
          <div className="h-full grid grid-rows-[auto_1fr] relative">
            <section
              id="header"
              className="relative w-full pt-6 pb-2 px-6 flex justify-between border-b-2 border-slate-300/50"
            >
              <div
                className="relative w-[inherit] flex gap-6"
                {...handlersHeader}
              >
                <div className="w-fit">
                  ...
                  <h1
                    className={`text-sm text-black/50 ${
                      subjectUpdatedData === "none" && "hidden"
                    }`}
                  >
                    อัพเดตล่าสุด: {subjectUpdatedData}
                  </h1>
              ...
              <div className="">
                <div className="flex gap-4">
                  <span
                    className="p-2 rounded-lg bg-slate-200/70 w-24 h-8 overflow-hidden flex gap-2 justify-center items-center border-b-2 border-slate-300 hover:bg-slate-300 hover:border-0 cursor-pointer"
                    onClick={() => fnHandleClickedOnFilter()}
                  >
                    <FontAwesomeIcon
                      icon={faLayerGroup}
                      style={{ color: "#73787e" }}
                    />{" "}
                    <span className="text-sm text-black/60">คัดกรอง</span>
                  </span>
                  <span
                    className="p-2 rounded-lg aspect-square bg-slate-200/70 w-8 h-8 overflow-hidden flex justify-center items-center border-b-2 border-slate-300 hover:bg-slate-300 hover:border-0 cursor-pointer"
                    onClick={() => toggleScheduleSpectate(false)}
                  >
                    <FontAwesomeIcon
                      icon={faClose}
                      style={{ color: "#73787e" }}
              ...
              <div
                className={`absolute top-full left-0 w-full h-fit max-h-[40dvh] overflow-auto pb-6 px-8 sm:grid grid-cols-2 gap-4 backdrop-blur-md z-10 bg-white/80 border-b-2 border-slate-300/30 smooth ${
                  !state.filter.popupToggle && "opacity-0 pointer-events-none"
                }`}
              >
                <div className="">
                  <div
                    className={`pt-6 ${filter.code.length > 0 && "opacity-50"}`}
                  >
                    หมวดหมู่รายวิชา
                    <span className="flex flex-wrap gap-2 items-center pt-1 relative">
                      {ge_subject_group_name.map((gsg, gindex) => (
                        <span
                          key={gindex}
                          onClick={() => fnHandleChangeFilterType(gsg.type)}
                          className={`smooth w-16 text-center ${
                            (filter.code.length == 0 &&
                              filter.type.includes(gsg.type)) ||
                            checkGroupCodes(gsg.code)
                              ? "bg-slate-700/60 text-white"
                              : "bg-slate-400/30"
                          } xl:hover:bg-slate-700/60 xl:hover:text-white px-2 py-1 rounded-lg ${
                            filter.code.length > 0
                              ? "pointer-events-none"
                              : "cursor-pointer"
                          } text-sm`}
                        >
                          หมวด {gsg.type.split("-")[1]}
                        </span>
                      ))}
                    ...
                    <span className={`${filter.code.length == 0 && "hidden"}`}>
                      <p className="text-black/70 pt-2 text-[13px]">
                        หากเลือกเฉพาะวิชา จะไม่สามารถเลือกหมวดหมู่รายวิชาได้
                      </p>
                    </span>
                    <span
                      className={`${
                        (filter.code.length != 0 || filter.date.length != 0) &&
                        "hidden"
                      }`}
                    >
                      <p className="text-black/40 pt-2 text-[13px]">
                        หากเลือกเฉพาะหมวดหมู่รายวิชาอย่างเดียว เว็บจะช้ามาก
                      </p>
                    </span>
                  </div>
                  <div className="pt-6">
                    เฉพาะวิชาที่เลือก
                    <span className="flex flex-wrap gap-2 items-center pt-1 relative">
                      {filter.code.map((gcode, gindex) => (
                        <span
                          key={gindex}
                          className={`${getColorCode(
                            gcode
                          )} px-2 py-1 rounded-lg cursor-pointer text-sm text-black/60`}
                          onClick={() => {
                            fnHandleToggleFilterSubjectCode(gcode);
                          }}
                        >
                          {" "}
                          {gcode}
                      ...
                      <span
                        className="w-16 text-center text-center bg-slate-400/30 xl:hover:bg-slate-700/60 xl:hover:text-white px-2 py-1 rounded-lg cursor-pointer text-sm"
                        onClick={() => {
                          toggleScheduleNameFilter(true);
                          updateSubjectGroupList();
                        }}
                      >
                        +
                      </span>
                ...
                <div className="">
                  <div className="pt-6">
                    วันที่เรียน
                    <span className="flex flex-wrap gap-2 items-center pt-1 relative">
                      {name_days.map((item, iin) => (
                        <span
                          key={iin}
                          onClick={() =>
                            fnHandleChangeFilterDate(
                              item.date_2,
                              checkFilterDateSelected(item.date_2)
                            )
                          }
                          className={`smooth w-16 text-center ${
                            checkFilterDateSelected(item.date_2)
                              ? "bg-slate-700/60 text-white"
                              : "bg-slate-400/30"
                          } xl:hover:bg-slate-700/60 xl:hover:text-white px-2 py-1 rounded-lg cursor-pointer text-sm`}
                        >
                          {item.date_th}
                        </span>
                      ))}
                  ...
                  <div className="py-6">
                    ...
                    <span className="flex flex-wrap gap-2 items-center pt-1 relative">
                      <span
                        className={`w-16 text-center bg-slate-400/30 xl:hover:bg-slate-700/60 xl:hover:text-white px-2 py-1 rounded-lg cursor-pointer text-sm ${
                          filter.time == "total" && "bg-slate-700/60 text-white"
                        }`}
                        onClick={() => {
                          fnHandleChangeFilterTime("total");
                        }}
                      >
                        ทั้งหมด
                      </span>
                      ...
                      >
                        <TimePicker
                          onChange={fnHandleChangeFilterTime}
                          value={filter.time === "total" ? "" : filter.time}
                          placeHolder={"เวลา"}
                          isOpen={state.filter.popupTimeToggle}
                        />
                      </div>
                    </span>
                    <span>
                      <p className="text-black/40 pt-2 text-[13px]">
                        สามารถเลือกกำหนดเวลาเรียนได้ โดยการกดที่ช่อง
                        &quot;เวลา&quot;
                      </p>
                    </span>
                  </div>
                  <div className="w-full flex justify-end gap-4">
                    <div className="border-2 border-black/20 rounded-md overflow-hidden">
                      <button
                        className="bg-white hover:bg-black/10 text-black/60 py-1 px-8 cursor-pointer"
                        onClick={() => {
                          fnHandleClearFilter();
                        }}
                      >
                    ...
                    <button
                      className="bg-slate-700 text-white rounded-md py-1 px-8 cursor-pointer"
                      onClick={() => {
                        fnHandleSendFilterUpdate();
                      }}
                    >
                      คัดกรอง
                    </button>
            ...
            <section
              id="subjects"
              className={`px-5 w-full h-full overflow-y-auto smooth ${
                state.filter.popupToggle && "blur-[2px]"
              }`}
              style={{
                opacity:
                  ((state.filter.popupDelay == -1 ||
                    state.filter.popupDelay == 3) &&
                    state.filter.popupToggle) ||
                  delay_id == -1
                    ? 1
                    : 1 - (state.filter.popupDelay + 1) / 3,
              }}
              onScroll={() => {
                if (state.filter.popupToggle) toggleScheduleFilter(false);
              }}
              onClick={() => {
                if (state.filter.popupToggle) toggleScheduleFilter(false);
              }}
              ...
              {!filter.firstFilter ? (
                <div className="w-full h-full flex flex-col justify-center items-center text-slate-400">
                  ...
                  <div className="pt-8 text-center">
                    ...
                    <p
                      className="underline cursor-pointer hover:text-slate-600 active:text-slate-700"
                      onClick={fnHandleClickedOnFilter}
                    >
                      คัดกรองข้อมูล
                    </p>
        ...
        <div
          className={`fixed smooth-out flex justify-center items-end w-full h-full ${
            state.filter.popupNameToggle && "bg-black/20"
          } z-50 rounded-t-3xl pointer-events-none`}
        >
          <div
            className={`smooth-out overflow-hidden pointer-events-auto fixed rounded-t-3xl w-[96%] h-[62dvh] bg-white ${
              state.filter.popupNameToggle ? "bottom-0" : "-bottom-full"
            }`}
          >
            <div className="h-full grid grid-rows-[auto_1fr] relative">
              <section
              ...
              >
                ...
                <div className="">
                  <div className="flex gap-4">
                    <span
                      className="py-2 px-4 rounded-lg bg-slate-200/70 overflow-hidden flex justify-center items-center border-b-2 border-slate-300 hover:bg-slate-300 cursor-pointer"
                      onClick={() => toggleScheduleNameFilter(false)}
                    >
                      ถัดไป
                    </span>
                  </div>
                ...
                <div
                  className={`absolute top-full left-0 w-full h-[4rem] overflow-auto grid-cols-2 gap-4 backdrop-blur-md z-10 bg-white/80 border-y-2 border-slate-300/30 smooth ${
                    state.filter.popupNameHeader == "" ? "hidden" : "sm:grid"
                  }`}
                >
                  <article className="pt-2 pb-2 px-5">
                    <h1 className="font-bold">
                      {state.filter.popupNameHeader}
                    </h1>
              ...
              <section
                id="subjects"
                onScroll={fnHandleFilterSubjectSelected}
                className={`px-5 w-full h-full overflow-y-auto smooth relative`}
              >
                {ge_subject_group_name.map((sgn, sgnindex) => (
                  <section
                    ...
                    id={"type-" + sgn.type}
                    className={`${
                      sgnindex + 1 == ge_subject_group_name.length && "mb-6"
                    }`}
                  >
                    <article className="pt-4 pb-2">
                      <h1 className="font-bold">
                        หมวดหมู่ที่ {sgn.code.substring(3, 4)}
                      </h1>
                    ...
                    {subjectGroupData[sgnindex]?.map((sjg, sjindex) => (
                      <div
                        key={sjindex}
                        className={`smooth border-b-2 border-slate-100 ${
                          checkFilterSubjectCodeContains(sjg.code) &&
                          "bg-green-200"
                        } rounded-md cursor-pointer`}
                        onClick={() => {
                          fnHandleToggleFilterSubjectCode(sjg.code);
                        }}
                      >
                        <button className="pl-2 w-full py-2 text-left flex gap-2 items-center">
                          <FontAwesomeIcon
                            className={`${getTextColorCode(sgn.code)}`}
                            icon={faCircle}
                          />
                          <p>
                            {sjg.code} - {sjg.name}
